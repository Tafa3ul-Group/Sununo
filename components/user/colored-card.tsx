...
import Svg, { Path, Defs, ClipPath, G, Rect, Image as SvgImage } from 'react-native-svg';
...
// SVG Path Data from assets/card/shaps/num1.svg to num5.svg
const SHAPE_PATHS = [
...
const STROKE_COLORS = ["#035DF9", "#FF7E4F", "#EA2129", "#EF79D7", "#15AB64"];
...
export function ColoredCard({
  ...
  // Match the stroke color from the asset
  const strokeColor = STROKE_COLORS[currentIndex];
  ...
  return (
    ...
    >
      {/* Favorite Button - White circle with heart icon */}
      <TouchableOpacity style={styles.favoriteButton}>
        <View style={styles.favoriteCircle}>
          <Ionicons name="heart" size={14} color="#EA2129" />
        </View>
      </TouchableOpacity>

      {/* Organic Shape Image - Centered and scaled to fit the smaller card */}
      <View style={styles.imageContainer}>
        <View style={styles.svgWrapper}>
          <Svg height="120" width="130" viewBox="0 0 140 135">
            <Defs>
              <ClipPath id={`clip-${currentIndex}`}>
                <Path d={pathData} />
              </ClipPath>
            </Defs>
            <G clipPath={`url(#clip-${currentIndex})`}>
               <SvgImage
                href={{ uri: image }}
                width="140"
                height="135"
