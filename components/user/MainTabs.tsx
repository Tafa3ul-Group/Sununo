...
// Explicitly creating the animated component for maximum compatibility on all devices
...
export const MainTabs: React.FC<MainTabsProps> = ({ activeTab, onChange }) => {
  ...
  // Shared value for transition (0 to 2)
  ...
  const animatedProps = useAnimatedProps(() => {
    const x = interpolate(transition.value, [0, 1, 2], [-112, 0, 112]);
    const s = interpolate(transition.value, [0, 1.0, 2], [0.82, 1, 0.82]);
    const fill = interpolateColor(
      transition.value,
      [0, 1, 2],
      [colors.WHO, colors.WHEN, colors.WHERE],
    );
  ...
  const whenTextStyle = useAnimatedStyle(() => ({
    color: interpolateColor(
      transition.value,
      [0.5, 1, 1.5],
      [colors.WHEN, "#FFFFFF", colors.WHEN],
    ),
    transform: [
