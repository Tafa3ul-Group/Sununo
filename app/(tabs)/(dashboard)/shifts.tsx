      ...
      {!pricing || !pricing.length ? (
        <View style={styles.emptyPricing}>
          ...
          <TouchableOpacity onPress={() => onEdit(pricing)} style={{ marginTop: 8 }}>
            <Text style={{ color: Colors.primary, fontFamily: "Alexandria-Bold" }}>{isRTL ? 'إعداد الأسعار' : 'Set Prices'}</Text>
          </TouchableOpacity>
        </View>
      ) : (
                ...
                >
                  <View style={{ padding: 20 }}>
                    ...
                    <View style={[styles.row, { justifyContent: 'center' }]}>
                      <Text style={{ color: Colors.primary, fontFamily: "Alexandria-Bold", fontSize: 13 }}>
            ...
            <View style={styles.formContainer}>
              {(!shiftForm.name || (!selectedShift && !shiftForm.price)) && (
                <View style={[styles.fullDayWarning, { backgroundColor: '#FFF9F9', borderColor: '#FFE0E0', padding: 12, marginBottom: 16 }]}>
                  ...
                  <Text style={{ fontSize: 13, color: '#FF3B30', fontFamily: "Alexandria-Bold", marginLeft: 8 }}>
              ...
              {!selectedShift && (
                <>
                  ...
                  <View style={[styles.row, { position: 'relative' }]}>
                    ...
                    />
                    <Text style={{ position: 'absolute', right: isRTL ? undefined : 16, left: isRTL ? 16 : undefined, top: 12, color: Colors.text.muted, fontFamily: "Alexandria-Bold" }}>
                      {isRTL ? 'د.ع' : 'IQD'}
                    </Text>
                  </View>
                </>
            ...
            {policyForm.map((policy, index) => (
              <View key={index} style={styles.policyFormCard}>
                <View style={[styles.rowInputs, { flexDirection, justifyContent: 'space-between', marginBottom: 16 }]}>
                  <Text style={{ fontFamily: "Alexandria-Bold", color: Colors.primary }}>{isRTL ? `قاعدة رقم ${index + 1}` : `Rule #${index + 1}`}</Text>
                  {policyForm.length > 1 && (
