    ...
    return (
      ...
      >
        ...
        <View style={styles.chaletInfo}>
          <Text style={styles.chaletName} numberOfLines={1}>{chaletName}</Text>
          <View style={styles.locationContainer}>
            <SolarIcon name="map-point-linear" size={12} color="#64748B" />
            <Text style={styles.locationText} numberOfLines={1}>{chaletLocation}</Text>
          </View>
        ...
        >
          ...
          <View style={styles.welcomeSection}>
            <View>
              ...
              <ThemedText style={styles.nameText}>{profile?.fullName || (user?.name || '...')}</ThemedText>
            </View>
            <TouchableOpacity 
          ...
          <View style={styles.statsGrid}>
            <View style={[styles.statsCard, { backgroundColor: '#F0F5FF' }]}>
              <View style={[styles.statsIconBox, { backgroundColor: '#DEE8FF' }]}>
                <SolarIcon name="banknote-2-linear" size={20} color="#035DF9" />
              </View>
              <ThemedText style={styles.statsLabel}>{t('dashboard.revenue.total')}</ThemedText>
              <View style={styles.balanceContainer}>
                 <ThemedText style={styles.statsValue}>{isBalanceVisible ? formatPrice(walletBalance) : '••••••'}</ThemedText>
                 <TouchableOpacity onPress={handleToggleBalance} style={styles.eyeBtn}>
                   <SolarIcon name={isBalanceVisible ? "eye-linear" : "eye-closed-linear"} size={18} color="#94A3B8" />
                 </TouchableOpacity>
              </View>
            </View>

            <View style={styles.statsRow}>
              <View style={[styles.statsCardSmall, { flex: 1, backgroundColor: '#FFF7ED' }]}>
                <View style={[styles.statsIconBox, { backgroundColor: '#FFEDD5' }]}>
                  <SolarIcon name="calendar-minimalistic-linear" size={18} color="#EA580C" />
                </View>
                <ThemedText style={styles.statsLabelSmall}>{t('tabs.bookings')}</ThemedText>
                <ThemedText style={styles.statsValueSmall}>{recentBookings.length || 0}+</ThemedText>
              </View>
              <View style={[styles.statsCardSmall, { flex: 1, backgroundColor: '#ECFDF5', marginLeft: 12 }]}>
                <View style={[styles.statsIconBox, { backgroundColor: '#D1FAE5' }]}>
                  <SolarIcon name="home-2-linear" size={18} color="#059669" />
                </View>
                <ThemedText style={styles.statsLabelSmall}>{isRTL ? 'الشاليهات' : 'Chalets'}</ThemedText>
                <ThemedText style={styles.statsValueSmall}>{chalets?.length || 0}</ThemedText>
              </View>
            </View>
          </View>
          ...
          <View style={styles.actionSection}>
            <View style={styles.sectionHeader}>
              <ThemedText style={styles.sectionTitle}>{isRTL ? 'إجراءات سريعة' : 'Quick Actions'}</ThemedText>
            </View>
            <View style={styles.actionGrid}>
               ...
               >
                 ...
                 <ThemedText style={styles.actionText}>{t('tabs.addChalet')}</ThemedText>
               </TouchableOpacity>
               ...
               >
                 ...
                 <ThemedText style={styles.actionText}>{t('tabs.revenue')}</ThemedText>
               </TouchableOpacity>
               ...
               >
                 ...
                 <ThemedText style={styles.actionText}>{isRTL ? 'الحجوزات' : 'Bookings'}</ThemedText>
               </TouchableOpacity>

               <TouchableOpacity 
                 style={styles.actionBtn}
                 onPress={() => router.push('/(tabs)/(dashboard)/provider-profile')}
               >
                 <View style={[styles.actionIconBox, { backgroundColor: '#10B981' }]}>
                   <SolarIcon name="settings-linear" size={24} color="white" />
                 </View>
                 <ThemedText style={styles.actionText}>{isRTL ? 'الإعدادات' : 'Settings'}</ThemedText>
               </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <ThemedText style={styles.sectionTitle}>{isRTL ? 'شاليهاتك' : 'My Chalets'}</ThemedText>
              <TouchableOpacity onPress={() => router.push('/(tabs)/(dashboard)/add-chalet')}>
                <ThemedText style={styles.viewAllText}>{isRTL ? '+ إضافة' : '+ Add'}</ThemedText>
              </TouchableOpacity>
            </View>
            
            {isLoading ? (
              <ActivityIndicator color={Colors.primary} size="large" style={{ margin: 20 }} />
            ) : chalets && chalets.length > 0 ? (
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chaletsList}>
                {chalets.map(renderChaletCard)}
              </ScrollView>
            ) : (
              <View style={styles.emptyState}>
                <SolarIcon name="home-search-linear" size={48} color="#94A3B8" />
                <ThemedText style={styles.emptyStateText}>{isRTL ? 'لا توجد شاليهات بعد' : 'No chalets added yet'}</ThemedText>
                <SecondaryButton 
