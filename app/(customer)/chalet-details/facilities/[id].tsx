...
const FacilityCard = ({ label, subtext, color, iconId, isRTL }: { label: string; subtext?: string; color: string; iconId?: string | null; isRTL: boolean }) => {
  ...
  return (
    <View style={[styles.cardContainer, { flexDirection: 'row-reverse' }]}>
       <View style={[styles.textSide, { alignItems: 'flex-start', [isRTL ? 'marginRight' : 'marginLeft']: 15 }]}>
...
const styles = StyleSheet.create({
  ...
  cardContainer: { 
    ...
    alignItems: 'center' },
  textSide: { flex: 1 },
  cardLabel: { fontSize: 16, fontFamily: "Alexandria-Medium", color: '#111827' },
  cardSubtext: { fontSize: 12, color: '#6B7280', marginTop: 4, fontFamily: "Alexandria-Medium" },
  iconSideScalloped: {
    width: 44,
    height: 44,
