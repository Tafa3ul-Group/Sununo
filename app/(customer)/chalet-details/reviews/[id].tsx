...
export default function ReviewsScreen() {
  ...
  const { data: reviewsResponse, isLoading } = useGetChaletReviewsQuery({
    chaletId,
    page: 1,
    limit: 20,
  });
  ...
  const filterOptions = [
    ...
    {
      label: isArabic ? "الاعلى التقييمات" : "Highest Rated",
      value: "highest",
    },
          ...
          <View style={styles.starsRow}>
            {[1, 2, 3, 4, 5].map((i) => (
              <SolarStarBold
                ...
                size={normalize(32)}
                color={
                  i <= Math.round(Number(averageRating)) ? "#15AB64" : "#E5E7EB"
                }
              />
            ))}
          </View>
            ...
            return (
              <View key={idx} style={styles.revCardFlat}>
                ...
                <View style={[styles.dateWrapper, { alignItems: "flex-end" }]}>
                  <ThemedText
                    style={[styles.dateTextLabel, { alignSelf: "flex-end" }]}
                  >
                    {rev.date}
                  </ThemedText>
                </View>
